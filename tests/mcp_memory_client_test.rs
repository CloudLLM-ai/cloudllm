...
async fn test_mcp_memory_client_with_custom_timeout() {
    let client = McpMemoryClient::with_timeout("http://127.0.0.1:3000".to_string(), 60);
...
async fn test_mcp_memory_client_different_endpoints() {
    let client1 = McpMemoryClient::new("http://localhost:8080".to_string());
    let client2 = McpMemoryClient::new("http://192.168.1.100:3000".to_string());
    let client3 = McpMemoryClient::new("http://0.0.0.0:5000".to_string());
...
async fn test_mcp_memory_client_timeout_values() {
    let client30 = McpMemoryClient::with_timeout("http://localhost:8080".to_string(), 30);
    let client60 = McpMemoryClient::with_timeout("http://localhost:8080".to_string(), 60);
    let client120 = McpMemoryClient::with_timeout("http://localhost:8080".to_string(), 120);
...
async fn test_mcp_memory_client_protocol_consistency() {
    let clients = vec![
        McpMemoryClient::new("http://localhost:8080".to_string()),
        McpMemoryClient::with_timeout("http://localhost:9000".to_string(), 45),
        McpMemoryClient::new("http://127.0.0.1:5000".to_string()),
    ];

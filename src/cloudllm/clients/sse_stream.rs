...
pub fn open_chat_completions_stream(
    ...
    Box::pin(async move {
        let retries = crate::clients::common::http_retry_attempts();
        ...
        let resp = loop {
            ...
            match req.json(&body).send().await {
                Ok(resp) => {
                    ...
                    return Err(Box::new(StreamError(err)) as Box<dyn Error + Send + Sync>);
                }
                Err(e) => {
...
impl SseParser {
    /// Create an empty incremental SSE splitter.
    pub fn new() -> Self {
        Self {
            pending: Vec::new(),
            data_lines: Vec::new(),
    ...
    /// Feed the next TCP/HTTP body fragment and return completed SSE `data` payloads.
